...
def detect_global_heartbeats(sEMG, fs):
    """
    The 'Captain' Function.
    Extracts Grid 1 & 6 (Chest), filters them, and finds the Global Heartbeat Timestamp.
    Returns: A boolean mask or list of peak indices representing the 'Kill Schedule'.
    """
    print("   [Captain] Detecting Global Heartbeat from Chest Grids...")
    
    # 1. Extract Chest Grids (0 and 5)
    chest_data = np.hstack([sEMG[:, 0:64], sEMG[:, 320:384]])
    
    # 2. Fast Filter (Just to see spikes)
    b, a = butter(N=2, Wn=[15, 100], btype='bandpass', fs=fs)
    filtered = filtfilt(b, a, chest_data, axis=0)
    
    # 3. Create a "Master Lead" (Root Mean Square of all chest channels)
    # This crushes noise and highlights the QRS complex which is present in all.
    master_lead = np.sqrt(np.mean(filtered**2, axis=1))
    
    # 4. Find Peaks (The Heartbeats)
    # Distance = 0.4s (Assuming < 150 BPM)
    # Height = 3x Median Noise
    peaks, _ = find_peaks(master_lead, height=np.median(master_lead)*3, distance=int(fs*0.4))
    
    return peaks

def analyze_envelope_refined(sources, fs, global_heart_peaks=None):
    """
    Refined with "Capture Rate" logic to catch messy heart signals.
    """
    ...
    sync_tol_samples = int(0.05 * fs_stat)
    
    if global_heart_peaks is not None:
    ...
    for i in range(sources.shape[1]):
        ...
        # 3. Peak Finding (Adaptive Threshold)
        signal_iqr = iqr(sig_env)
        floor = np.median(sig_env)
        # Lower threshold slightly to catch weak heartbeats in noise
        peak_thresh = floor + 2.5 * signal_iqr 
        peaks, _ = find_peaks(sig_env, height=peak_thresh, distance=int(fs_stat*0.4))
        
        # 4. METRIC: Regularity
        if len(peaks) > 3:
            regularity = np.std(np.diff(peaks) / fs_stat)
        else:
            regularity = 10.0

        # 5. METRIC: Capture Rate (The "Recall" Score)
        # What % of the Global Heartbeats appeared in this component?
        capture_rate = 0.0
        if len(scaled_global_peaks) > 0 and len(peaks) > 0:
            match_count = 0
            for g_peak in scaled_global_peaks:
                # Is there a local peak near this global peak?
                dist = np.min(np.abs(peaks - g_peak))
                if dist < sync_tol_samples:
                    match_count += 1
        ...
        # A. We have Global Data (The preferred path)
        if len(scaled_global_peaks) > 0:
            # Case 1: High Capture Rate (> 50%). 
            # It pulses with the heart at least half the time. It's contaminated.
            if capture_rate > 0.50:
                c_type = 'HEART'
                # Massive boost to Kurtosis score to ensure it ranks #1 for removal
                kurt += 500 
                candidates.append({'id': i, 'kurt': kurt, 'type': c_type})
            
            # Case 2: Moderate Capture (> 20%) AND Very Spiky (> 3.0)
            # It's a sparse heart artifact.
            elif capture_rate > 0.20 and kurt > 3.0:
                c_type = 'HEART'
                kurt += 100
                candidates.append({'id': i, 'kurt': kurt, 'type': c_type})
                
        # B. Fallback (No Global Data found - maybe chest grids are noisy)
        else:
            if (0.7 <= dom_freq <= 3.0) and (kurt > 1.5):
                c_type = 'HEART' if regularity < 0.20 else 'ARTIFACT'
                candidates.append({'id': i, 'kurt': kurt, 'type': c_type})

    # Sort: Highest Kurtosis (modified by our boosts) gets deleted first
...
class EMGControlPanel:
    ...
    def run_processing_task(self):
        ...
        try:
            ...
            # Run this synchronously on the thread (fast enough)
            heart_timestamps = detect_global_heartbeats(self.sEMG_data, self.fs)
            print(f"Global Detection Found {len(heart_timestamps)} heartbeats.")
            ...
            for i in range(6):
                start, end = i * 64, (i + 1) * 64
                chunk = self.sEMG_data[:, start:end]
                # Pass Global Schedule to each task
                tasks.append((chunk, self.fs, i+1, heart_timestamps))

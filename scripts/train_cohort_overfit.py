#!/usr/bin/env python3
"""
Train one pooled multi-participant scalar AUX[0] overfit experiment.
"""
...
from src.training import run_cohort_overfit_experiment
from src.training.single_recording import collect_runtime_info, validate_runtime_requirements


def main() -> None:
    parser = argparse.ArgumentParser(description="Train a pooled cohort-level EMG -> AUX[0] overfit experiment")
    parser.add_argument("--recordings_dir", action="append", required=True)
    parser.add_argument(
        "--model",
        required=True,
    ...
    result = run_cohort_overfit_experiment(
        recordings_dirs=args.recordings_dir,
